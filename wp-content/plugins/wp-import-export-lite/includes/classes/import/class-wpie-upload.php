...
class WPIE_Upload {
        ...
        protected function wpie_manage_import_file($fileName = "", $fileDir = "", $wpie_import_id = 0) {
                ...
                $filePath = WPIE_UPLOAD_IMPORT_DIR . "/" . $fileDir . "/original";
                ...
                if (!is_file($current_file)) {
                        ...
                        return new \WP_Error('wpie_import_error', __('Uploaded file must be XML, CSV, ZIP, XLS, XLSX, XML, ODS, TXT, JSON', 'wp-import-export-lite'));
                        ...
                        $file_list = $this->wpie_get_file_list($filePath . "/extract", true, false);
                                ...
                                if (isset($file_list["config.json"])) {
                                        ...
                                        if ($_is_success) {
                                                ...
                                                if (is_array($config)) {
                                                        ...
                                                        $new_key = $this->wpie_create_safe_dir_name("config");

                                                        if (isset($config['fileName']) && !empty($config['fileName'])) {

                                                                $_new_filename = preg_replace("/[^a-z0-9\_\-\.]/i", '', $config['fileName']);

                                                                $_temp_path = WPIE_UPLOAD_IMPORT_DIR . "/" . $fileDir . "/original/extract/";
                ...
                if ($wpie_import_id > 0) {
                        ...
                        $wpie_import_id = $wpie_import->wpie_generate_template(array("importFile" => $file_data, "activeFile" => $active_file), 'import-draft', 'processing');
                }

                unset($filePath, $file_data, $active_file, $wpie_import);

                return array('file_list' => $fileList, 'wpie_import_id' => $wpie_import_id, "file_name" => sanitize_file_name($fileName), "file_size" => filesize($current_file));
        }
