...
class WPIE_BG_Import extends \wpie\import\WPIE_Import {
    ...
    public function wpie_bg_import() {
        ...
        $template = $wpdb->get_row("SELECT `id` FROM " . $wpdb->prefix . "wpie_template where `opration` in ('import','schedule_import') and status LIKE '%background%' and process_lock = 0 ORDER BY `id` ASC limit 1");

        if ($template && isset($template->id) && absint($template->id) > 0) {

            parent::wpie_import_process_data($template->id);
        }
        unset($template);
    }
